<?php @session_start();
    ...
    @include_once('../../classes/Seguro.class.php');

	$db = new Database();
    $seguroClass = new Seguro();

    // somente mostra o registros de consultas de 30 dias.
    $db->query = "SELECT COUNT(*) as total 
        FROM operador.tbl_atualiza_corretagem as f, credauto.consultas AS c
    ...
    $db->query = "SELECT f.id, f.consulta as cod_consulta, f.date_remocao, f.date_remocao AS data, c.ValorItem AS parametro, t.tipoconsulta AS tipo, t.id AS tipo_consulta, c.ItemConsultado as tipo_parametro , f.justificar, a.NomeAtendente AS nome, a.slack_id
        FROM operador.tbl_atualiza_corretagem AS f, credauto.consultas AS c, credauto.atendentes AS a, credauto.ttipoconsulta AS t 
...
>
    <form class="card d-flex flex-column h-100"
        data-form-type="ajax"
        data-form-target="#historico_recall"
        data-form-replace="true"
        action="<?=$baseURL?>/pages/historico_rf_alterados.page.php"
        method="get"
    >
        ...
        <div class="card-header d-flex flex-row align-items-center justify-content-between p-3 border-bottom">
            ...
            >
                <button type="submit" class="btn btn-light btn-sm"
                    formaction="<?=$baseURL?>/pages/atualizar_seguro/historico.php?pagina=<?= $paginaAtual ?>"
                >
            ...
            <?php if (!count($rows)): ?>
                <div class="d-flex flex-column align-items-center justify-content-center flex-fill p-5">
                    ...
                    <p class="text-center text-muted" style="max-width: 18em;">
                        Ainda não há nenhuma cotação de seguro que foi atualizada
                    </p>
                </div>
            <?php endif; ?>
            ...
            ?>
                <div class="list-group-item ">
                    ...
                    <div class="d-flex flex-row w-100">
                        ...
                        <img class="rounded me-3 mt-1"
                            src="<?=$baseURL?>/profile_image.php?fullname=<?= ucwords($row->nome) ?>" 
                            data-srcset="<?=$baseURL?>/profile_image.php?user=<?= $row->slack_id?>" 
                        ...
                        <div class="d-flex flex-column flex-grow-1">
                            <div class="d-flex w-100 justify-content-between mb-1">
                                ...
                                <div class="d-flex flex-column pe-1 flex-grow-1 ">
                                    <p class="mb-1">
                                        <?= ucwords($row->nome) ?> atualizou o Histórico RF
        ...
        <div class="card-footer">
            ...
            <div class="d-flex align-items-center justify-content-end">
                ...
                >
                    <button type="submit" class="btn btn-light btn-sm"
                        formaction="<?=$baseURL?>/pages/atualizar_seguro/historico.php?pagina=<?= $paginaAtual-1 ?>"
